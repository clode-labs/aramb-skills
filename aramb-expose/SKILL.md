...
Surface the primary frontend URL as a URL-kind artifact on your task close (in-task) or on a `aramb_mcp.chat_deliver_artifacts` call (solo). The platform auto-registers the preview-URL state. Secondary backend / API URLs go in the inline reply text alongside.
...
# Solo / mid-task recall: same artifact shape via aramb_mcp.chat_deliver_artifacts.
...
- A URL-kind artifact on `aramb_mcp.tasks_update.artifacts` (in-task) or `aramb_mcp.chat_deliver_artifacts.artifacts` (solo) is mandatory for the primary frontend URL.
