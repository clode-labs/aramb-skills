...
Load this file when the agent needs to chain tool calls, script workflows, or use `experimental_subAgent()` inside `composio run`.

## Use `run` To Chain Tool Calls

Fetch from multiple connected tools in one script:

If the agent does not need script logic and just wants a few independent calls, prefer top-level parallel execute instead:
...
`proxy(toolkit)` returns a fetch-compatible function bound to the connected account. It returns a real `Response`, so call `.json()` or `.text()`.
