...
Once ALL URLs are verified:

1. Update preview URL (primary frontend):
   ```
   npx mcporter call brahmi.update_preview_url project_id="<PROJECT_ID>" application_id="<APPLICATION_ID>" url="<FRONTEND_URL>"
   ```

2. Report all URLs to main chat:
   ```
