...
- **`assigned_agent` per node** — one agent per role, decided IDENTICALLY in solo and team (see "Per-node persona — decided by the work"). For each node: reuse an existing agent that fits the role (a roster persona — `developer` / `*-tester` / `checker` / `*-deployer`), otherwise **author a bespoke sub-agent spec INLINE in the workflow's `agent_specs`** named for its role (`issue-triager`, `fix-implementer`, `qa-tester`, `pr-author`, …) to the template-grade bar, and set the node's `assigned_agent` to that spec's `name`. In task dispatch, you may default to the source task's persona. A single-role workflow keeps `agent_specs` empty and points every node at **the agent's own routing name** (the `benji_agent_id` from `aramb_agents.get`, e.g. `inbox-digest-e1709fcf`) — never the literal `"main"`, and never `"master"`/`"solo"`, which are infra agents and would run the workflow as something other than this agent's persona — only for a trivial single-node / pure-glue workflow. Do NOT branch on solo vs team.
