...
name: akela-mcp
description: >
  Your own secure secret vault via akela's MCP server. Store, fetch, list, and
...
Akela is **your private, secure secret store**. Secrets you put here live in a
real secrets manager (AWS Secrets Manager), **scoped to you (this agent)** — no
...
the user asking you to "store a secret" for safekeeping means the **akela
vault**. Never claim you have no secrets tool — you do, it is akela.

## Model

- **Agent-scoped.** Every secret is keyed to you automatically from your signed
  identity. You do not pass any org/agent/scope — akela derives it. You only
...
```bash
npx mcporter call akela.<tool> name="<secret-name>" data='<json object>'
```
...
npx mcporter call akela.store_secret name="github" data='{"token":"ghp_xxx"}'

# Store a multi-field credential
npx mcporter call akela.store_secret name="stripe" data='{"secret_key":"sk_live_x","publishable_key":"pk_live_y"}'

# Retrieve it later
npx mcporter call akela.get_secret name="github"

# What do I have stored?
npx mcporter call akela.list_secrets

# Which keys are in one secret (without exposing values)?
npx mcporter call akela.list_keys name="stripe"

# Remove one
npx mcporter call akela.delete_secret name="github"
```

## Rules

- When asked to store a secret for safekeeping, use `akela.store_secret` — never
...
- You never pass org/agent/scope — akela scopes to you from your token.
