...
- `--backend claude-sdk` is the platform default — use it unless explicitly told otherwise. A new agent should match the deployment's default backend so its behaviour is consistent with every other agent in the workspace.
