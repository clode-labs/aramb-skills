...
2. Acknowledge: `npx mcporter call aramb_mcp.tasks_update project_id="<PROJECT_ID>" task_id="<TASK_UUID>" status="in_progress"`
3. Ping the user's main chat with a start notice (your reply text lands in the task chat — `aramb_mcp.chat_send_message chat_location="main"` is how main chat sees you started):
   `npx mcporter call aramb_mcp.chat_send_message project_id="<PROJECT_ID>" application_id="<APP_ID>" content="🔨 Starting: <task>" chat_location="main"`
4. Execute the work. Use additional `aramb_mcp.chat_send_message` calls for important mid-flight milestones; keep them short.
5. Close the task with the deliverable (chip + status + preview state, all in one call). Do NOT call `aramb_mcp.chat_send_message` after this close — the close already emits the chip-bearing row.
...
On failure: `npx mcporter call aramb_mcp.tasks_update project_id="<PROJECT_ID>" task_id="<TASK_UUID>" status="failed" error="what went wrong"`
On blockers: `npx mcporter call aramb_mcp.tasks_update project_id="<PROJECT_ID>" task_id="<TASK_UUID>" status="blocked" outputs='{"summary":"waiting on X"}'`
...
- **the workflow backend** — task management (aramb_mcp.tasks_update, aramb_mcp.chat_deliver_artifacts, aramb_mcp.chat_send_message, aramb_mcp.chat_ask_question)
