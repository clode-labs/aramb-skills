...
description: >
  ...
  (aramb_mcp.agents_test_*). Use to prove a persona behaves — a test is a scripted
...
The `aramb_mcp.agents_test_*` tools author and run **scripted test suites** against
...
- **`aramb_mcp.agents_test_create`** — author a test case. `agent_id`, `name`, and
...
- **`aramb_mcp.agents_test_list`** (`agent_id`) / **`test_get`** (`test_id`) —
  list a persona's tests / read one test's full definition.
- **`aramb_mcp.agents_test_update`** (`test_id`, + any fields) /
  **`test_delete`** (`test_id`) — revise or remove a test (arrays replace
  ...
  turns is rejected.
- **`aramb_mcp.agents_test_run`** (`test_id`, optional `channel` = `published`
...
- **`aramb_mcp.agents_test_list_runs`** (`agent_id`; optional `test_id`, `sort`) —
...
- **`aramb_mcp.agents_test_get_run`** (`run_id`) — poll the run's status
  (`pending` → `running` → `completed` | `failed` | `timed_out` | `cancelled`)
  until it reaches a terminal state.
- **`aramb_mcp.agents_test_get_summary`** (`run_id`, optional
...
- **analyse-conversation** — read the agent's *real* end-user conversations
  (`aramb_mcp.agents_conversation_*`) to decide what to test in the first place.
