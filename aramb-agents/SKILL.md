...
name: aramb-agents
description: >
  MCP toolkit for the organization's aramb agents (aramb_mcp.agents_*). Use to
  ...
  scripted test suites use the agent-tests skill (both are aramb_mcp.agents_*).
  NOT for provisioning workflow-node sub-agents — that is the create-agent skill.
---

# Aramb Agents Toolkit

The `aramb_mcp.agents_*` tools manage the organization's **product agents** — the
...
- **The design is settled and the user wants a NEW agent to exist** →
  `aramb_mcp.agents_create`. Create only AFTER the persona is agreed — name and
...
- **You are already operating on / within an existing agent** (an agent is already
  in context, being edited or built out) → **`aramb_mcp.agents_update`, NOT `create`.**
...
`aramb_mcp.agents_create` / `update` carry the whole persona, not just one prompt
...
npx mcporter call aramb_mcp.agents_create name="Support Triage" \
...
npx mcporter call aramb_mcp.agents_update agent_id="<AGENT_ID>" soul="You are warmer now — open with a short acknowledgement before triaging…"
```
...
still the `aramb_mcp.agents_*` toolkit; they were split out so each is a focused,
self-contained playbook:

- **Read the agent's real conversations** (evaluate from evidence → improve) →
  the **analyse-conversation** skill (`aramb_mcp.agents_conversation_search` /
...
- **Author and run scripted test suites** against a persona → the
  **agent-tests** skill (`aramb_mcp.agents_test_*`). Tests must script at least 3
  user turns.
...
## Knowledge Base — list / add / remove docs (`aramb_mcp.agents_kb_*`)
...
- **`aramb_mcp.agents_kb_add`** (`agent_id`, `filename`, `content`, optional `folder`) —
...
- **`aramb_mcp.agents_kb_remove`** (`agent_id`, `doc_id`) — remove one KB document by the
...
npx mcporter call aramb_mcp.agents_kb_list agent_id="<AGENT_ID>"
npx mcporter call aramb_mcp.agents_kb_add agent_id="<AGENT_ID>" filename="refund-policy.md" content="# Refund policy\n\nRefunds are honored within 30 days of purchase." folder="policies"
npx mcporter call aramb_mcp.agents_kb_remove agent_id="<AGENT_ID>" doc_id="<DOC_ID>"
```

## Export the agent as a reusable template (`aramb_mcp.agents_export_template`)

- **`aramb_mcp.agents_export_template`** (`agent_id`, `slug`, `name`, optional
...
```bash
npx mcporter call aramb_mcp.agents_export_template agent_id="<AGENT_ID>" slug="support-triage" name="Support Triage Agent" description="Triages inbound support and routes to the right queue." category="support" tags="support,triage" publish_first=true include_knowledge_doc_ids="<DOC_ID_1>,<DOC_ID_2>"
```
...
- **`aramb_mcp.agents_list_mcp_connections`** (`agent_id`) — list the agent's connected
...
npx mcporter call aramb_mcp.agents_list_mcp_connections agent_id="<AGENT_ID>"
npx mcporter call aramb_mcp.agents_export_template agent_id="<AGENT_ID>" slug="support-triage" name="Support Triage Agent" mcp_required='{"echo_server": true}'
```
...
`aramb_mcp.agents_create` / `update`. Declaring a port is not enough to run: each
...
npx mcporter call aramb_mcp.agents_update agent_id="<AGENT_ID>" required_toolkits='["GMAIL"]'
```
...
`aramb_mcp.toolkits_connect` / `execute GITHUB_GET_GIT_CREDENTIAL`
...
- **The agent must touch an external service** (Gmail, Drive, Slack, a sheet) →
  ...
  (`aramb_mcp.toolkits_search` → `get_schema` → `execute`). Name the concrete
...
- A **bound** workflow belongs to **exactly one agent** — and there are **two
  ...
  `aramb_mcp.workflows_create` (create-and-link in one call). **Workflow-first:** if the
  ...
  `aramb_mcp.agents_attach_workflow` once the agent exists (its `agent_id` gets stamped and
...
- A workflow stays a **draft** on creation; the builder tests it via Preview. There
  ...
  version **automatically when you `aramb_mcp.agents_publish` the owning agent**. So
...
- **Publishing a toolkit-using workflow is gated on its toolkits being connected.**
  ...
  `aramb_mcp.toolkits_check_connection` and tell the builder which toolkits to connect —
