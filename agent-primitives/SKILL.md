...
- **Supporting tools / params** — the exact `aramb_*` tool or `aramb_agents.create/update` field.
...
> All agent create/inspect/revise/publish goes through `aramb_agents.*` only — never a CLI
...
- **Supporting tools / params:** `aramb_agents.create` / `update` → `system_prompt` (+ `system_prompt_mode`: replace|append), `soul` (personality/voice), `agents_doc` (operating playbook), `name`.
...
- **Supporting tools / params:** text/markdown YOU author → `aramb_agents.kb_add` (`kb_list` / `kb_remove` to manage). Binaries/PDFs → the user uploads them (you can't add inline).
...
- **Supporting tools / params:** discover with the **aramb-skills** skill (`aramb-skills search`); attach via `aramb_agents.update` → `skills` (registry ids, max 20). Never invent an id.
...
- **Supporting tools / params:** ground slugs with `aramb_toolkits.list_toolkits`; record on the agent as `required_toolkits` (uppercase catalog slugs). The USER connects each account.
...
- **Supporting tools / params:** `aramb_agents.create` / `update` → `browser: true`. This injects the browser usage guideline so the agent uses its built-in browser skill for the web (rather than a raw fetch/search).
...
- **Supporting tools / params:** the connection (URL + write-only, multi-header credentials) is created by the USER in the console and proxied so the secret never enters the container. You can **read** an agent's connected servers with `aramb_agents.list_mcp_connections`; when exporting a template, mark each required/optional via `mcp_required`.
...
- **Where:** Configure → **Tools → MCP** tab (the user adds the server + its credentials here).
...
- **Supporting tools / params:** `aramb_agents.create` / `update` → `tools` (an explicit ALLOWLIST that *replaces* the default toolset) and `disabled_tools` (remove specific tools from the default profile).
...
- **Supporting tools / params:** `aramb_agents.create` / `update` → `voice: true`. A capability marker only — it has no prompt or dispatch effect on its own.
...
- **Supporting tools / params:** the workflow skills (`aramb_workflows.create` / `update`, create-workflow / import-workflow). Distinct-role nodes → author each role's sub-agent spec INLINE in the `agent_specs` array.
...
- **Supporting tools / params:** `aramb_workflows.update` → the workflow's `instruction` (how/when to invoke, what input) + `enabled: true`.
...
- **Supporting tools / params:** `aramb_agents.trigger_create` / `trigger_list` (kinds: cron, webhook, toolkit_event).
...
- **Supporting tools / params:** the vault skill → `vault.create_platform_secret` (a write-only placeholder the USER fills). You never see or ask for the real value.
...
- **Supporting tools / params:** `aramb_agents.add_guardrail` / `list_guardrails` / `update_guardrail` / `remove_guardrail`, plus `set_default_guardrail` and `set_guardrails_config` (toggle the platform defaults + the master switch).
...
- **Supporting tools / params:** the agent-tests skill → `aramb_agents.test_create` / `test_run` / `test_get_run` / `test_get_summary`. Each test scripts ≥3 user turns.
...
- **Supporting tools / params:** `aramb_agents.publish` (proof = `published_version`).
...
- **Supporting tools / params:** read real chats with `aramb_agents.conversation_search` / `conversation_get` (analyse-conversation skill) to ground improvements.
