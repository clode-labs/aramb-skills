...
5. Check for pending tasks via `aramb_mcp.tasks_list`
...
1. **ALWAYS look up the application's existing workflow first.** This is unconditional. Run this lookup every turn, regardless of what you remember from earlier in the chat:
   ...
   **The chat is not the source of truth — the database is.** Workflow rows get deleted between turns. Tasks you remember dispatching may have completed asynchronously. Status may have changed. Even if you "just" dispatched an update task and feel certain it's still in flight, **verify**, don't assume. The cost of a redundant lookup is one tool call; the cost of acting on stale memory is doing nothing while telling the user you did something.
   ...
   Likewise, before claiming "an update is already in flight" or "task X is still running", call `aramb_mcp.tasks_list status="in_progress"` and confirm. If the task is `done`, treat the user's new prompt as a fresh request, not a refinement.
...
3. **Workflow exists** → call:
   ```bash
   npx mcporter call aramb_mcp.workflows_update_from_tasks workflow_id="<WORKFLOW_ID>" change_request="<USER'S EXACT INSTRUCTION>"
   ```
...
After dispatching either tool, send a one-line confirmation to the user via `aramb_mcp.chat_send_message` so the chat shows you've kicked it off — e.g. *"Starting workflow consolidation, task <id>."* — then STOP. The dispatched system task will arrive separately; do not start designing in this turn.
...
3. After the skill returns, send a one-line confirmation via `aramb_mcp.chat_send_message` showing the resulting cron + timezone + `next_run_at`.
...
skill — NOT `schedule-workflow`. It reads the trigger catalog with `aramb_mcp.toolkits_*`
and persists a `toolkit_event` row with `aramb_mcp.triggers_*`.

1. Look up the workflow first (`aramb_mcp.workflows_get application_id="<APPLICATION_ID>"`).
...
9. Submit via `aramb_mcp.tasks_create`
...
**Task completion** — the sub-agent's `aramb_mcp.tasks_update` close call carries the deliverable (the sub-agent passes its `project_id` and `task_id` from its dispatch User Message):
...
**On-the-fly recall** — when the user asks about something the agent already produced ("show me that report you made earlier"), the sub-agent calls `aramb_mcp.chat_deliver_artifacts`:
...
- **aramb_tasks / aramb_workflows / aramb_chat** — task lifecycle (`aramb_mcp.tasks_create`, `aramb_mcp.tasks_update`, `aramb_mcp.tasks_list`), workflow lifecycle (`aramb_mcp.workflows_*`), and chat surface (`aramb_mcp.chat_send_message`, `aramb_mcp.chat_ask_question`)
...
2. **Every task description includes completion instructions** — agents must know how to report back via `aramb_mcp.tasks_update` with their explicit `task_id` (rendered into their dispatch User Message)
